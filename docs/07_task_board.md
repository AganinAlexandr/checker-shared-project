...
| TSK-006 | Шаг 1.1 | Проверка существенности битой кодировки | active | opus | `docs/01_plan.md`, `reason_codes.md`, `item_level_toc.broken_encoding` | Правило по ЧИСЛУ затронутых пунктов: 1-2 → локальные замечания + продолжать; >2/существенно → общее замечание (Приказ 783) + огранич. режим. В коде есть `section_error="broken_encoding"` (порог сегментов >50) — заменить на пунктовую существенность + reason_code + метрика |
