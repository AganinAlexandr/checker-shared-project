...
`[AGENT-OK]` — проверено против пересобранного артефакта (d792d66).

- **ИД исключён из модели авторства, сохранён в инвентаре** — верная граница (данные не теряем, анализ чистим). Секции в наборе: КР 129/ПОС 133/СМ 131/ИНЖ 4/АР 1, **ИД отсутствует**; 398 пар (−10 ИД).
- **H1 воспроизведён точно:** residual near-shingle same 0.3559/diff 0.0394, strong-share 0.7851/0.0727. Сигнал устойчив (≈9× shingle, 10.8× strong-share) — почти не сдвинулся (ИД было 10 пар).
- H2 без изменений.

**ИТОГ:** GIP-control H1 теперь на полностью очищенном корпусе (без ПЗ, без ОВ-substring, без ИД-дампа) — **сигнал почерка ГИП чистый, широкий (КР/ПОС/СМ) и сильный**, H2-вывод (GIP-специфичный перенос) устойчив. Подготовка/валидация закрыты. **СЛЕД:** добор по data_gaps (Сергеев фасад/АР same-org, Корнеева, новые субподряд-ГИП) и/или потребительские UC поверх валидированного сигнала.
